...
# resample to participant mask (which in not in MNI)
brain_AAL = image.resample_to_img(
                        brain_AAL,
                        participants_mask,
                        interpolation='nearest')
...
for ind, roi_name in enumerate(list(ROI_mask_paths.keys())[:-2]):
    ...
    intersect_mask = masking.intersect_masks(masks, threshold=1, connected=False) # make sure all voxels were present in the HCP team results
...
plt.savefig(os.path.join(figures_dir, "masking", "check_masking_AAL.pdf"))
plt.close("all")
...
# Load HCP
resampled_maps = numpy.load(os.path.join(data_path, "resampled_maps.npy".format(1)), allow_pickle=True)
masker_GM_WM = NiftiMasker(
    mask_img=participants_mask)
masker_GM_WM.fit(resampled_maps)
resampled_maps_per_team = masker_GM_WM.inverse_transform(resampled_maps)
...
GM_data = masker_GM.fit_transform(resampled_maps_per_team)
...
for roi_name in list(ROI_mask_paths.keys())[:-2]:
    ...
    extracted_data = masker_roi.fit_transform(resampled_maps_per_team)
...
for roi_name in list(ROI_mask_paths.keys()):
    ...
    masker_roi = NiftiMasker(
                mask_img=nib.load(ROI_mask_paths[roi_name]))
    resampled_maps_in_ROI = masker_roi.fit_transform(resampled_maps_per_team)
    ...
    for i, roi_name in enumerate(list(ROI_mask_paths.keys())[:-1]):
        ...
        if "GLS" in SDMA_method:
            if "Consensus" in SDMA_method:
                ...
                axs[i, 0].axhline(y=3.5, color='black', linestyle='--', linewidth=0.5)
                axs[i, 0].axhline(y=-3.5, color='black', linestyle='--', linewidth=0.5)
                axs[i, 0].set_title("{}".format(roi_name))
                axs[i, 0].set_ylim(-7.5, 7.5)

        else:
            axs[i, 0].axhline(y=0.75, color='black', linestyle='--', linewidth=0.5)
            axs[i, 0].axhline(y=-0.75, color='black', linestyle='--', linewidth=0.5)
            axs[i, 0].set_title("{}".format(roi_name))
