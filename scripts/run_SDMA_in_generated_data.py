...
for sim in simulations:
    ...
    generated_data = {"Null":data_generator.generate_simulation(case="Null", K=K, J=J, corr=correlation), 
                    "Null correlated 80%": data_generator.generate_simulation(case="Null correlated", corr=correlation, K=K, J=J),
                    "Null mix": data_generator.generate_simulation(case="Null mix", corr=correlation, K=K, J=J)}
    # utils.plot_generated_data(generated_data, results_dir)

    # utils.paper_figure_simulations(generated_data, results_dir)
    ...
    for simulation in generated_data.keys():
        ...
        for row in range(K):
            for MA_model in MA_outputs.keys():
                df_weights[MA_model] = MA_outputs[MA_model]['weights'].squeeze()
        ...
        print(" "*100)
        # for Poster
        if simulation in ["Null", "Null correlated 80%", "Null mix"]:
    ...
    utils.plot_PP_final_figure_2(Poster_results, results_dir, correlation, J, K)