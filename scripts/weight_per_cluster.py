...
pipeline_z_scores_per_team = numpy.load('{}/data/Hyp{}_resampled_maps.npy'.format(results_dir, hyp), allow_pickle=True).item()
...
def plot_distance_from_highly_correlated_cluster_for_one_voxel(pipeline_z_scores, team_names, results_dir, clusters_name, clusters, which_voxel):
	plt.close("all")
	print("get highly correlated pipelines mean")
	highly_correlated_pipelines = ["3TR7", "Q6O0", "E3B6", "L7J7", "9Q6R", "U26C", "50GV", "B5I6", "R9K3", "C88N", 
				"J7F9", "46CD", "C22U", "I52Y", "E6R3", "R7D1", "0C7Q", "6VV2", "98BT", "6FH5", "3C6G", "L3V8", "0I4U",
				"0H5E", "9U7M"]
	highly_correlated_pipelines_full_name = get_full_name(highly_correlated_pipelines)
	print(highly_correlated_pipelines_full_name)
	highly_correlated_pipelines_indices = get_cluster_indices([highly_correlated_pipelines_full_name], ["highly_correlated"], team_names)
	...
	_, contributions_GLS = utils.compute_contributions(pipeline_z_scores, std_by_Stouffer=False)
	correlated_cluster_indices = highly_correlated_pipelines_indices["highly_correlated"]
	correlated_cluster_mean = pipeline_z_scores[correlated_cluster_indices, which_voxel].mean()
	dist_pipelines_mean_from_correlated_mean = []
	for pipeline, weight in enumerate(weight_pipelines_gls):
		dist_pipeline_mean_from_correlated_mean = pipeline_z_scores[pipeline, which_voxel] - correlated_cluster_mean
	...
	sns.regplot(x=dist_pipelines_mean_from_correlated_mean, y=contributions_GLS[:, which_voxel],ci=None, ax=ax0, color='black')
	...
	for ind, dist in enumerate(dist_pipelines_mean_from_correlated_mean):
		for i, name in enumerate(clusters_name):
			if team_names[ind] in clusters[i]:
				ax0.plot(dist, contributions_GLS[:, which_voxel][ind], 'o', markersize=6, color=c[i], alpha=0.5)
				ax1.plot(dist, pipeline_z_scores[:, which_voxel][ind], 'o', markersize=6, color=c[i], alpha=0.5)
	plt.tight_layout()
	plt.savefig("{}/hyp1/{}clusters_results/contribution_per_distance_voxel_{}.png".format(results_dir,len(clusters_name), which_voxel))



def plot_distance_from_highly_correlated_cluster(pipeline_z_scores, team_names, results_dir, clusters_name, clusters, eight_voxels_nicelly_defined):
	plt.close("all")
	...
	_, contributions_GLS = utils.compute_contributions(pipeline_z_scores, std_by_Stouffer=False)
	correlated_cluster_indices = highly_correlated_pipelines_indices["highly_correlated"]
	correlated_cluster_mean = pipeline_z_scores[correlated_cluster_indices].mean()
	dist_pipelines_mean_from_correlated_mean = []
	for pipeline, weight in enumerate(weight_pipelines_gls):
		dist_pipeline_mean_from_correlated_mean = pipeline_z_scores[pipeline, :].mean() - correlated_cluster_mean
		dist_pipelines_mean_from_correlated_mean.append(numpy.abs(dist_pipeline_mean_from_correlated_mean))
	...
	for ind, dist in enumerate(dist_pipelines_mean_from_correlated_mean):
		for i, name in enumerate(clusters_name):
			if team_names[ind] in clusters[i]:
				...
				ax6.plot(dist, pipeline_z_scores.mean(axis=1)[ind], 'o', markersize=6, color=c[i], alpha=0.5)
	...
	for voxel_significance_profil in eight_voxels_nicelly_defined.keys():
		voxel_ind = eight_voxels_nicelly_defined[voxel_significance_profil]
		contributions_GLS_for_4_voxels.append(contributions_GLS[:, voxel_ind])
		original_zscore_for_4voxels.append(pipeline_z_scores[:, voxel_ind])
		dist_pipelines_from_correlated_mean_for_this_voxel = []
		correlated_cluster_mean_for_this_voxel = pipeline_z_scores[correlated_cluster_indices][:, voxel_ind].mean()
		for pipeline, _ in enumerate(weight_pipelines_gls):
			dist_pipeline_from_correlated_mean_of_this_voxel = pipeline_z_scores[pipeline, voxel_ind] - correlated_cluster_mean_for_this_voxel
	...
	ax2.set_xlabel("Distance, {}".format(list(eight_voxels_nicelly_defined.keys())[0]))
	...
	ax7.set_xlabel("Distance, {}".format(list(eight_voxels_nicelly_defined.keys())[0]))
	...
	ax3.set_xlabel("Distance, {}".format(list(eight_voxels_nicelly_defined.keys())[1]))
	...
	ax8.set_xlabel("Distance, {}".format(list(eight_voxels_nicelly_defined.keys())[1]))
	...
	ax4.set_xlabel("Distance, {}".format(list(eight_voxels_nicelly_defined.keys())[2]))
	...
	ax9.set_xlabel("Distance, {}".format(list(eight_voxels_nicelly_defined.keys())[2]))
	...
	ax5.set_xlabel("Distance, {}".format(list(eight_voxels_nicelly_defined.keys())[3]))
	...
	ax10.set_xlabel("Distance, {}".format(list(eight_voxels_nicelly_defined.keys())[3]))
	...
	plt.tight_layout()
	plt.savefig("{}/hyp1/{}clusters_results/contribution_per_distance.png".format(results_dir,len(clusters_name)))


def plot_distance_from_highly_correlated_cluster_absolute(pipeline_z_scores, team_names, results_dir, clusters_name, clusters, eight_voxels_nicelly_defined):
	plt.close("all")
	print("getting cluster indices...")
	clusters_indices = get_cluster_indices(clusters, clusters_name, team_names)
	print("get highly correlated pipelines mean")
	...
	plt.tight_layout()
	plt.savefig("{}/hyp1/{}clusters_results/contribution_per_distance_absolute.png".format(results_dir,len(clusters_name)))


def plot_brains_final_figure(clusters, clusters_name, team_names, masker, pipeline_z_scores):
	print("Check using the original computation of GLS contributions:")
	T_map, p_map, _ = MA_estimators.GLS_SDMA(pipeline_z_scores)
	...
	print("get SDMA Stouffer and GLS contributions...")
	contributions_SDMA_Stouffer, contributions_GLS = utils.compute_contributions(pipeline_z_scores, W="SDMA", std_by_Stouffer=False)
...
# plot reconstructed brain SDMA Stouffer contribution
	...
	print("Saving plot")
	plt.savefig("{}/hyp1/{}clusters_results/per_cluster_weights_{}.png".format(results_dir,len(clusters_name), "{}clusters".format(len(clusters_name))))
	plot_distance_from_highly_correlated_cluster(pipeline_z_scores, team_names, results_dir, clusters_name, clusters, eight_voxels_nicelly_defined)
	plot_distance_from_highly_correlated_cluster_absolute(pipeline_z_scores, team_names, results_dir, clusters_name, clusters, eight_voxels_nicelly_defined)
...
def plot_voxels_per_cluster(masker, pipeline_z_scores, team_names, results_dir, clusters_name, clusters, eight_voxels_nicelly_defined):
	...
	for voxel_significance_profil in eight_voxels_nicelly_defined.keys():
		...
		plt.tight_layout()
		title = "{}/hyp1/{}clusters_results/cluster_for_voxel_{}.png".format(results_dir, len(clusters_name), voxel_index)
		plt.savefig(title)
		# plt.show()
...
def compute_GLS_weights_for_shrinked_nb_of_anticorrelated_pipelines(clusters, clusters_name, team_names, masker, shrinked_pipeline_z_scores, nb_anticorr_pipes_to_remove):
	print("Check using the original computation of GLS contributions:")
	T_map, p_map, _ = MA_estimators.GLS_SDMA(shrinked_pipeline_z_scores)
	T_map_GLS_nii = masker.inverse_transform(T_map)
	p_map = (p_map <= 0.05) * T_map
	p_map_GLS_nii = masker.inverse_transform(p_map)
	...
	print("get SDMA Stouffer and GLS contributions...")
	_, contributions_GLS = utils.compute_contributions(shrinked_pipeline_z_scores, W="SDMA", std_by_Stouffer=False)
	mean_contributions_GLS_nii = masker.inverse_transform(numpy.mean(contributions_GLS, axis=0))
	...
	plt.savefig("{}/hyp1/{}clusters_results/effect_of_removing_{}anticorr_pipelines_on_per_cluster_weights_{}.png".format(results_dir,len(clusters_name), nb_anticorr_pipes_to_remove, "{}clusters".format(len(clusters_name))))
	# plt.show()
...
############################
### voxel well defined with correlated and anti_correlated:
...
# for which_voxel in random.sample(range(0, pipeline_z_scores.shape[1]), 5):
# 	print("Plotting random voxel..")
# 	plot_distance_from_highly_correlated_cluster_for_one_voxel(pipeline_z_scores, team_names, results_dir, clusters_name, clusters, which_voxel)

plot_brains(clusters, clusters_name, team_names, masker, pipeline_z_scores)
plot_voxels_per_cluster(masker, pipeline_z_scores, team_names, results_dir, clusters_name, clusters, eight_voxels_nicelly_defined)
